...
class Game {
    ...
    /** This is the stack that saves the previous states
     *  of the game in order. */
    private Stack<Game> state = new Stack<Game>();
    ...
    /** Creates a copy of the current game and
     * pushes the game into the state stack.
     * This method is applied to every card move.  */
    void pushToStack() {
        Game copy = new Game(this);
        state.push(copy);
    }

    /** The undo method copies the previous state to the current state and
     * pops it off the state stack.
     */
    void undo() {
        if (state.size() > 1) {
            copyFrom(state.pop());
